...
class MotionDataParser():
    ...
    @property
    def n_ric_data_cols(self) -> int:
        return (self.joints_num - 1) * 3 + 4
    
    @property
    def n_rot_data_cols(self) -> int:
        return (self.joints_num - 1) * 6
    
    @property
    def root_r_velocity_slice(self) -> slice:
        return slice(0, 1)
    
    @property
    def root_l_velocity_slice(self) -> slice:
        return slice(1, 3)
    
    @property
    def root_y_slice(self) -> slice:
        return slice(3, 4)

    @property
    def ric_slice(self) -> slice:
        return slice(4, self.n_ric_data_cols)
    
    @property
    def feet_l_slice(self) -> slice:
        return slice(-4, -2)
    
    @property
    def feet_r_slice(self) -> slice:
        return slice(-2, None)

    @property
    def rot_slice(self) -> slice:
        start_idx = self.n_ric_data_cols
        end_idx = start_idx + self.n_rot_data_cols
        return slice(start_idx, end_idx)
    
    def get_ric_data(self, data: np.ndarray) -> np.ndarray:
        return data[:, self.ric_slice]
    
    def get_rot_data(self, data: np.ndarray) -> np.ndarray:
        return data[:, self.rot_slice]
    
    def get_root_r_velocity(self, data: np.ndarray) -> np.ndarray:
        return data[:, self.root_r_velocity_slice]
    
    def get_root_l_velocity(self, data: np.ndarray) -> np.ndarray:
        return data[:, self.root_l_velocity_slice]
    
    def get_root_y(self, data: np.ndarray) -> np.ndarray:
        return data[:, self.root_y_slice]
    
    def get_feet_r(self, data: np.ndarray) -> np.ndarray:
        return data[:, self.feet_r_slice]
...
def task_status(task_id):
    task = generate_sample_task.AsyncResult(task_id)
...
def tasks():
    ...
    for task_info in task_infos:
        ...
        task = generate_sample_task.AsyncResult(task_info["task_id"])
        tasks_status.append({
            "task_id": task_info["task_id"],
            "text_prompt": task_info["text_prompt"],
            "status": task.state
        })
    return json.dumps(tasks_status)
...
@app.route("/model")
def model():
    model_path = Path("./save/humanml_trans_enc_512/model000200000.pt")
    print(model_path)
